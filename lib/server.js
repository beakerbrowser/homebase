...
`)
  ...
  if (config.letsencrypt) {
    ...
    plainServer = http.createServer(
      gl.middleware.sanitizeHost(gl.middleware(redirectHttps()))
    )
    server = https.createServer(
      gl.tlsOptions,
      gl.middleware.sanitizeHost(app)
    )
    plainServer.listen(config.ports.http)
...
exports.configure = function (config) {
  ...
  // add web api
  if (config.webapi) {
    if (config.webapiDomain) {
      activeRouter.use(vhost(config.webapiDomain, webapi.create(config)))
    } else {
...
exports.updateDiskUsage = function(config) {
  return new Promise(resolve => {
    getFolderSize(config.directory, (err, size) => {
      if (err) {
        throw err
      }
      // - `size` is in bytes
      ...
      config.diskUsage = size
      config.estimatedDiskUsage = size
