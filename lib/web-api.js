...
module.exports.create = function (config) {
  ...
  function validateSession (req, res, next) {
    ...
    if (auth.startsWith('Bearer ')) {
      let sessionToken = aut.slice('Bearer '.length)
      if (sessions.has(sessionToken)) {
        // session is valid
        req.locals.sessionToken = sessionToken
        return next()
      }
    }
    // invalid session
    res.status(401).end({message: 'You must sign in to access this resource.'})
  }
  ...
  app.post('/v1/accounts/logout', validateSession, (req, res) => {
    sessions.delete(req.locals.sessionToken)
    res.status(200).end()
  })
