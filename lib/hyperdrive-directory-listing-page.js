...
module.exports = renderDirectoryListingPage
...
async function renderDirectoryListingPage (drive, dirPath) {
  // list files
  var names = []
  try { names = await drive.promises.readdir(dirPath) } catch (e) {}

  // stat each file
  var entries = await Promise.all(names.map(async (name) => {
    var entry
    var entryPath = joinPaths(dirPath, name)
    try { entry = await drive.promises.stat(entryPath) } catch (e) { return false }
