...
class HomebaseDatConfig {
  ...
  get hostnames () {
    var hostnames = []
    if (this.name && this.config.webapiDomain) {
      hostnames.push(`${this.name}.${this.config.webapiDomain}`)
    }
    return hostnames.concat(this.domains || [])
  }
...
function validateDatCfg (dat) {
  ...
  if (dat.otherDomains && !dat.domains) {
  ...
  if (dat.name) check(typeof dat.name === 'string', 'dats.*.name must be a string, see https://github.com/beakerbrowser/homebase/tree/master#datsname', dat.name, 'invalidName')
  if (dat.domains) {
