...
NOTE 2: the `id` field is used to identify config (eg in the dats vhost)
NOTE 3: the `checksum` field is used to detect changes, so the fields used in that will determine what triggers a reconfiguration
...
class HomebaseConfig {
  ...
  writeToFile (configPath = false) {
    configPath = configPath || this.configPath
    fs.writeFileSync(configPath, yaml.safeDump(this.canonical))
  }

  getVhost (checksum) {
    return this.allVhosts.find(v => v.checksum === checksum)
  }

  isActiveVhost (checksum) {
    return Boolean(this.getVhost(checksum))
  }
  ...
  updateDat (datKey, datCfg) {
    ...
    if (datCfg.otherDomains) {
      oldDatCfg.otherDomains = datCfg.otherDomains
    } else {
      delete oldDatCfg.otherDomains
    }
    // write
    this.writeToFile()
  }
...
class HomebaseDatConfig {
  ...
  get checksum () {
    return checksum(_pick(this, ['datKey', 'hostnames']))
  }
...
class HomebaseProxyConfig {
  ...
  get checksum () {
    return checksum(_pick(this, ['from', 'to']))
  }
...
function validateDatCfg (dat) {
  ...
  check(typeof dat.name === 'string', 'dats.*.name must be specified, see https://github.com/beakerbrowser/homebase/tree/master#datsname', dat.name, 'invalidName')
  if (dat.otherDomains) {
...
function validate (config) {
  ...
  if (config.dats) {
    ...
    config.dats.forEach(validateDatCfg)
  }
  if (config.proxies) {
