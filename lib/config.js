...
class HomebaseDatConfig {
  ...
  get hostnames () {
    var hostnames = []
    if (this.config.webapiDomain) {
      hostnames.push(`${this.name}.${this.config.webapiDomain}`)
    }
    return hostnames.concat(this.otherDomains || [])
  }
...
function validate (config) {
  ...
  if ('domain' in config) {
    ...
    if (config.webapi && typeof config.webapi === 'object' && !('domain' in config.webapi)) {
      // move onto the webapi object
  ...
  if (config.webapi) check(typeof config.webapi.domain === 'string', 'webapi.domain must be specified, see https://github.com/beakerbrowser/homebase/tree/master#webapidomain')
  if (config.dats) {
